...
public class SystemService extends BaseService implements InitializingBean {
	 ...
	 * @return
	 */
	public Office getOfficeByPhone(String phone) {
		return officeDao.getByPhone(phone);
	}
...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void saveAgentUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			userDao.insert(user);
		}
		if (StringUtils.isNotBlank(user.getId())){
			// 新增代理商用户角色
			userDao.insertAgentUserRole(user);
			// 将当前用户同步到Activiti
			saveActivitiUser(user);
			// 清除用户缓存
			UserUtils.clearCache(user);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean validatePassword(String plainPassword, String password) {
		String plain = Encodes.unescapeHtml(plainPassword);
		System.out.println("plainPassword===="+plainPassword+"       password======"+password);
