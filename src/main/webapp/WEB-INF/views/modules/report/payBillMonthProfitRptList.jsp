...
<ul class="nav nav-tabs">
		<li class="active"><a href="javascript:;">历史分润统计</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="payBillMonthProfitRpt" action="${ctx}/report/payBillMonthProfitRpt/list" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="type" name="type" type="hidden"/>
		<ul class="ul-form">
			<li><label>所属机构：</label><sys:treeselect id="officeId" name="officeId" value="${payBillMonthProfitRpt.office.id}" labelName="office.name" labelValue="${payBillMonthProfitRpt.office.name}" 
			...
			<li><label>月份：</label>
				<form:input id="month" path="month" type="text" readonly="readonly" maxlength="6" class="input-medium Wdate"
					value="${payBillMonthProfitRpt.month}"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="payBillMonthProfitRpt">
			<tr>
				...
				<td>
					<a href="javascript:;" onclick="openWin(1200,600,'${ctx}/report/payBillMonthProfitRpt/profitDetail?month=${payBillMonthProfitRpt.month}&officeId=${payBillMonthProfitRpt.office.id}');">查看明细</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
