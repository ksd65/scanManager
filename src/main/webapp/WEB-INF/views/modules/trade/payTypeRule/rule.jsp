...
<head>
	...
	<script type="text/javascript">
		...
		function getRouteMerchantList(oid,mid){
			//$("#"+mid).text("");
			...
			$.ajax({
				...
				success:function(data) {
					...
					for(var i=0;i<list.length;i++){
						subHtml = subHtml+ "<option value=\""+list[i].merchantCode+"\">"+list[i].merchantName+"("+list[i].merchantCode+")"+"</option>";	
					}
					$("#"+mid).html(subHtml);
				},
				error : function(XMLHttpRequest, textStatus, errorThrown) {    
...
<body>
	...
	<form:form id="inputForm" modelAttribute="payTypeRule" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<input id="btnSubmit" class="btn btn-primary" type="button" onclick="saveMem()" value="保 存"/>&nbsp;
