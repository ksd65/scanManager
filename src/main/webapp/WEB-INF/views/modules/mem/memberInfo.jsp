...
<head>
	...
	<script type="text/javascript">
		...
		function saveMem(){
			...
			$.ajax({  
				url:"${ctx }/mem/member/updateMem",
				data:{memberId:$("#memberId").val(),name:name,shortName:$.trim($("#shortName").val()),contact:$.trim($("#contact").val()),mobilePhone:mobilePhone,
				certNbr:$.trim($("#certNbr").val()),busLicenceNbr:$.trim($("#busLicenceNbr").val()),email:$.trim($("#email").val()),
				remarks:$.trim($("#remarks").val()),tradeInfo:str},
				type:'post',
				cache:false,
				async:false,
				dataType:'json',
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="">商户详情</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="member" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<table class="table table-bordered info-table" align="center" id="mtb">
        	...
        	<form:hidden path="id" id="memberId"/>
	        <tr>
			...
			<tr>
				...
				<td width="30%">
					...
					title="机构" url="/sys/office/treeData" disabled="disabled"  />
			...
			<tr>
				...
				<td width="30%">
					<input id="t0DrawFee" name="t0DrawFee" class="input-medium" type="text" value="" maxlength="9" onkeyup="formatMoneyWith2digts(this);">元
				</td>
				<td class="info-table-label"><div class="text-right">T1提现手续费:</div></td>
				<td width="30%">
					<input id="t1DrawFee" name="t1DrawFee" class="input-medium" type="text" value="" maxlength="9" onkeyup="formatMoneyWith2digts(this);">元
				</td>
			</tr>
			</tbody>
			...
			<c:forEach items="${paytype }" var="plist" varStatus="status">
				...
				<tbody class="pay" id="${status.index }">
			        <tr>
						<td class="info-table-label"><div class="text-right">交易方式:</div></td>
						<td width="30%">
							<select id="txnMethod_${status.index }" name="txnMethod" class="input-medium">
								<c:forEach items="${fns:getDictList('txn_method')}" var="mlist">
									<c:choose>
										<c:when test="${plist.payMethod==mlist.value }">
											<option value="${mlist.value }" selected="selected">${mlist.label }</option>
										</c:when>
										<c:otherwise>
											<option value="${mlist.value }">${mlist.label }</option>
										</c:otherwise>
						...
						<td class="info-table-label"><div class="text-right">交易类型:</div></td>
						<td width="30%">
							<select id="txnType_${status.index }" name="txnType" class="input-medium">
								<c:forEach items="${fns:getDictList('txn_type')}" var="mlist">
									<c:choose>
										<c:when test="${plist.txnType==mlist.value }">
											<option value="${mlist.value }" selected="selected">${mlist.label }</option>
										</c:when>
										<c:otherwise>
											<option value="${mlist.value }">${mlist.label }</option>
			        ...
			        <tr>
						<td class="info-table-label"><div class="text-right">D0交易费率:</div></td>
						<td width="30%">
							<input id="t0TradeRate_${status.index }" name="t0TradeRate" class="input-medium" type="text" value="${plist.t0TradeRate }" maxlength="9" onkeyup="formatMoneyWith4digts(this);">
						</td>
						<td class="info-table-label"><div class="text-right">T1交易费率:</div></td>
						<td width="30%">
							<input id="t1TradeRate_${status.index }" name="t1TradeRate" class="input-medium" type="text" value="${plist.t1TradeRate }" maxlength="9" onkeyup="formatMoneyWith4digts(this);">
						</td>
					</tr>
					<tr>
						<td class="info-table-label"><div class="text-right">D0提现手续费:</div></td>
						<td width="30%">
							<input id="t0DrawFee_${status.index }" name="t0DrawFee" class="input-medium" type="text" value="${plist.t0DrawFee }" maxlength="9" onkeyup="formatMoneyWith2digts(this);">元
						</td>
						<td class="info-table-label"><div class="text-right">T1提现手续费:</div></td>
						<td width="30%">
							<input id="t1DrawFee_${status.index }" name="t1DrawFee" class="input-medium" type="text" value="${plist.t1DrawFee }" maxlength="9" onkeyup="formatMoneyWith2digts(this);">元
